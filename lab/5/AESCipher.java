/**
 * @author Michael Guarino
 * @course MSCS 630 Lab 5
 */

public class AESCipher{

  // declare constants

  // SBOX in hex
  private static final char[] SBOX = {
    
  };

  // RCON in hex
  private static final char[] RCON = {
    
  };

  // Mixed cols Galois Field substitutions table 2
  private static final int[] MC2 = {
    
  };

  // Mixed cols Galois Field substitutions table 3
  private static final int[] MC3 = {
    
  };

  private static final int NIBBLE_LENGTH = 2;
  private static final int INPUT_LENGTH = 4;
  private static final int NUM_ROUNDS = 11;
  private static final int NUM_COLs = NUM_ROUNDS * INPUT_LENGTH;
  private static final int RADIX = 16;

  /**
  * @desc generate 10 rounds of keys from initial user defined input hex key
  * @param KeyHex - the initial hex key input by the user as a string
  * @return roundKeysHex - 10 rounds of generated keys as 2D array
  */
  public static String aesRoundKeys(String KeyHex){

    if (((int) Math.sqrt(KeyHex.length()/NIBBLE_LENGTH))!=INPUT_LENGTH){
      System.out.println("Input has invalid length");
      System.exit(0);
    }

    // convert key to 4x4 matrix
    String[][] outHex = create4x4Matrix(KeyHex);

    // generate round keys
    String[][] roundKeys = AESKeygen(outHex);

    String roundKeysHex = "";
    for(int i=0; i<NUM_ROUNDS; i++){
      for(int j=(i*INPUT_LENGTH); j<(i*INPUT_LENGTH)+INPUT_LENGTH; j++){
        for(int k = 0; k<INPUT_LENGTH; k++){
          roundKeysHex += roundKeys[k][j].toUpperCase();
        }
      }
      roundKeysHex +="\n";
    }
    return roundKeysHex;
  }

  /**
  * @desc encrypt plaintext with AES
  * @param pTextHex - plaintext to encrypt
  * @param keyHex - key used for encryption
  * @return c - cipher text encrypted with AES
  */
  public static String AES(String pTextHex, String keyHex){

    // check that key input is valid length
    if (((int) Math.sqrt(keyHex.length()/NIBBLE_LENGTH))!=INPUT_LENGTH){
      System.out.println("Input has invalid length");
      System.exit(0);
    }

    // convert plaintext to 4x4 matrix
    String[][] pTextHex_2D = create4x4Matrix(pTextHex);
    // generate AES key
    String[][] w = AESKeygen(create4x4Matrix(keyHex));
    // convert key to 4x4 matrix
    String[][] roundKey = create4x4Matrix(keyHex);
    // create 4x4 matrix for ciphertext
    String[][] c = new String[4][4];

    // first xor
    c = AESStateXOR(pTextHex_2D, roundKey);

    // perform AES for 10 rounds
    for(int i=1; i<11; i++){
      for(int j=0; j<4; j++){
        for(int k=0; k<4; k++){
          roundKey[k][j] = w[k][(i*4)+j];
        }
      }

      // shift rows and nibble sub
      c = AESShiftRow(AESNibbleSub(c));

      if (i<10){
        //mixed cols except for final round
        c = aesMixColumn(transpose(c));
      }

      // perform xor on ciphertext with roundKey
      c = AESStateXOR(c, roundKey);
    }

    // format cipher text to display
    StringBuilder sb = new StringBuilder();
    for (String[] i:transpose(c)){
      for(String j:i){
        sb.append(j);
      }
    }
    return sb.toString().toUpperCase();
  }

  /**
  * @desc create 2d matrix of dim 4x4 from hex string
  * @param KeyHex - the initial hex key input by the user as a string
  * @return outHex - converted initial hex key 2D array
  */
  private static String[][] create4x4Matrix(String KeyHex){
    int row = 0;
    int col = 0;
    String[][] outHex = new String[INPUT_LENGTH][INPUT_LENGTH];

    // add two chars together to form one hex character
    for (int i=0; i<KeyHex.length(); i+=2){
      outHex[row][col] = "" + KeyHex.charAt(i) + KeyHex.charAt(i+1);
      row++;

      if (row==4){
        row = 0;
        col++;
      }
    }
    return outHex;
  }

  /**
  * @desc generate 10 rounds of keys from initial user defined input hex key as 2D array
  * @param outHex - initial hex key 2D array
  * @return genKey - generated Key as 2D array
  */
  private static String[][] AESKeygen(String[][] outHex){

    String[][] genKey = new String[outHex[0].length][NUM_COLs];

    for (int i=0; i<NUM_COLs; i++){

      // first 4 rounds
      if (i < INPUT_LENGTH){
        genKey[i][0] = outHex[i][0];
        genKey[i][1] = outHex[i][1];
        genKey[i][2] = outHex[i][2];
        genKey[i][3] = outHex[i][3];
      }

      // rounds after round 4
      else{

        // col multiple of 4
        if (i % INPUT_LENGTH == 0){

          String[] genKey_prime = new String[INPUT_LENGTH];
          String RCONValue = aesRcon(i);

          // convert hex to int and xor
          genKey_prime[0] = xor(Integer.parseInt(RCONValue, RADIX),
                        Integer.parseInt(aesSBox(Integer.parseInt(genKey[1][i-1], RADIX)), RADIX));

          // convert hex to int and get SBOX sub
          genKey_prime[1] = aesSBox(Integer.parseInt(genKey[2][i-1], RADIX));
          genKey_prime[2] = aesSBox(Integer.parseInt(genKey[3][i-1], RADIX));
          genKey_prime[3] = aesSBox(Integer.parseInt(genKey[0][i-1], RADIX));

          for (int j=0; j<INPUT_LENGTH; j++){
            genKey[j][i] = xor(Integer.parseInt(genKey[j][i-INPUT_LENGTH], RADIX),
                          Integer.parseInt(genKey_prime[j], RADIX));
          }
        }

        else{
          for (int j=0; j<INPUT_LENGTH; j++){
            // convert hex to int and xor
            genKey[j][i] = xor(Integer.parseInt(genKey[j][i-INPUT_LENGTH], RADIX),
                          Integer.parseInt(genKey[j][i-1], RADIX));
          }
        }
      }
    }
    return genKey;
  }

  /**
  * @desc existing hex value substituted with value based on round
  * @param round - current round in key generation process
  * @return substitued round value
  */
  private static String aesRcon(int round){
    // get RCON value depending on round
    return Integer.toHexString((int) RCON[(int) Math.floor(round/4)]).toUpperCase();
  }

  /**
  * @desc existing hex value substituted with SBox value
  * @param outHex - character being substituted
  * @return substituted value from sbox
  */
  private static String aesSBox(int outHex){
    return Integer.toHexString((int) SBOX[outHex]).toUpperCase();
  }

  /**
  * @desc xors two strings input
  * @param L - int to be xored substituted
  * @param R - int to be xored substituted
  * @return xored value
  */
  private static String xor(int L, int R){
    // perform bitwise xor, format, and convert bitwise xor to string
    return String.format("%02x", L^R);
  }

  /**
  * @desc perform xor on sHex with keyHex
  * @param sHex - input string to be xored
  * @param keyHex - input key to be used to xor sHex
  * @return result - xored result
  */
  public static String[][] AESStateXOR(String[][] sHex, String[][] keyHex){
    String[][] result = new String[4][4];
    for(int i=0; i<4; i++){
      for(int j=0; j<4; j++){
        result[i][j] = xor(Integer.parseInt(sHex[i][j], RADIX), Integer.parseInt(keyHex[i][j], RADIX));
      }
    }
    return result;
  }

  /**
  * @desc perform nibble sub for aes
  * @param inStateHex - input string sub with SBOX
  * @return result - nibble sub with SBOX applied
  */
  public static String[][] AESNibbleSub(String[][] inStateHex){
    String[][] result = new String[4][4];
    for(int i=0; i<4; i++){
      for(int j=0; j<4; j++){
        result[i][j] = aesSBox(Integer.parseInt(inStateHex[i][j], RADIX));
      }
    }
    return result;
  }

  /**
  * @desc perform shift of rows for aes
  * @param inStateHex - 4x4 matrix to be shifted
  * @return result - shifted results
  */
  public static String[][] AESShiftRow(String[][] inStateHex){
    String[][] result = new String[4][4];
    result[0] = inStateHex[0];
    for(int i=1; i<4; i++){
      for(int j=0; j<4; j++){
        result[i][j] = inStateHex[i][(j+i)%4];
      }
    }
    return result;
  }

  /**
  * @desc transpose matrix aka flip rows and cols
  * @param array_2D - 4x4 square matrix aka 2D array
  * @return result - the transposed matrix
  */
  private static String[][] transpose(String[][] array_2D){
    String[][] result = new String[4][4];
    for(int i=0; i<4; i++){
      for(int j=0; j<4; j++){
        result[i][j] = array_2D[j][i];
      }
    }
    return result;
  }

  /**
  * @desc mix cols with MC2 and MC3 boxes
  * @param inStateHex - 4x4 matrix to be mixed
  * @return result mix column operation
  */
  public static String[][] aesMixColumn(String[][] inStateHex_transpose){
    String[][] result = new String[4][4];
    int[] a = new int[4];
    int[] r = new int[4];

    // convert to hex to int
    for (int i=0; i<4; i++){
      for (int j=0; j<4; j++){
        a[j] = Integer.parseInt(inStateHex_transpose[i][j], RADIX);
      }

      // do col mixing
      r[0] = MC2[a[0]]^MC3[a[1]]^a[2]^a[3];
      r[1] = a[0]^MC2[a[1]]^MC3[a[2]]^a[3];
      r[2] = a[0]^a[1]^MC2[a[2]]^MC3[a[3]];
      r[3] = MC3[a[0]]^a[1]^a[2]^MC2[a[3]];

      // convert col mixed int to string
      for (int k=0; k<4; k++){
        result[i][k] = String.format("%02x", r[k]);
      }
    }
    return transpose(result);
  }
}